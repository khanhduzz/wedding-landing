...
export default function WeddingInvitationCard({ dict }: { dict: any }) {
  const [isOpen, setIsOpen] = useState(false);
  const leafColors = ["#BC8A5F", "#A27B5C", "#D4A373"];

  // Shortcut truy cập invitation dict
    ...
    <section className="relative py-16 md:py-32 bg-[#FAF7F2] flex flex-col items-center overflow-hidden min-h-[750px]">
      ...
      >
        ...
        <h2 className="text-4xl md:text-6xl font-light text-[#3D3831] tracking-tight">
          <span className="font-serif italic text-[#BC8A5F]">{inv.title}</span>
        </h2>
      </motion.div>
      ...
      <div className="mt-16 flex justify-center">
        ...
        >
          <div
            className={`absolute inset-0 w-full h-full bg-[#3D3831] transition-transform duration-500 ease-in-out ${isOpen ? "translate-y-0" : "translate-y-full group-hover:translate-y-0"}`}
          />
          <div className="absolute inset-0 w-full h-full border border-[#3D3831] rounded-full" />
          <span
            className={`relative z-10 text-xs font-bold tracking-[0.3em] uppercase transition-colors duration-500 ${isOpen ? "text-[#FAF7F2]" : "text-[#3D3831] group-hover:text-[#FAF7F2]"}`}
          >
